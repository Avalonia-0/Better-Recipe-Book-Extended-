 ...
 *  can be added manually via {@code config/brbe_workstations.json}. */
public final class WorkstationExporter {
    ...
    private static WorkstationSpec toSpec(Identifier uid, Set<Identifier> items) {
        if (!uid.getNamespace().equals("minecraft")) {
            BetterRecipeBook.LOGGER.info("[BRBE-JEI-Plugins] skipping modded recipe type {} (add manually via config/brbe_workstations.json)", uid);
            return null;
        }
        String family;
        ...
        switch (uid.getPath()) {
            ...
            default -> {
                BetterRecipeBook.LOGGER.info("[BRBE-JEI-Plugins] skipping unknown recipe type {} (add manually via config/brbe_workstations.json)", uid);
                return null;
            }
        }

...
public final class ViewerTransfer {
    ...
    private static void moveStack(MultiPlayerGameMode gameMode, AbstractContainerMenu menu,
        ...
        gameMode.handleInventoryMouseClick(menu.containerId, from, 0, ClickType.PICKUP, player);
        gameMode.handleInventoryMouseClick(menu.containerId, targetSlot, 0, ClickType.PICKUP, player);
        if (!menu.getCarried().isEmpty()) {
            ClientInventoryUtil.storeItem(-1, i -> i >= 3);
        }
    }

...
public final class TopLayerOverlayRenderer {
    ...
    public static void render(Screen screen, GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        // of the frame (the platform's after-render event) and raises the
        // stratum itself, so an unconditional host-group draw lands ABOVE the
        // window — which the screen's own RETURN hook drew earlier (2026-09-13
        // user report: the alternative-recipe group covered the query window).

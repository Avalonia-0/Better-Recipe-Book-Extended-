    ...
    public static void refresh() {
        ...
        try {
            ...
            for (Identifier typeId : typeIds) {
                ...
                // registerType 覆盖——只把 headless 收集的 JEI 原生 layout 按
                // display 匹配挂到 BRBE 已注册条目上，供弹窗委托完整 JEI UI。
            ...
            if (total > 0) {
                ...
                BrbeLogger.log("BRBE-JEI-BRIDGE", "imported {} JEI entries from headless-jei ({} types)",
                        total, typeIds.size());
            }
    ...
    private static void attachVanillaLayouts(Identifier typeId,
        ...
        try {
            ...
            for (Object entry : entries) {
                ...
                // RecipeHolder.value() 返回 Recipe（SmithingRecipe），不是
                // RecipeDisplay；display 需经 Recipe.display() 提取（26.2 实机
                // 日志证实 holder.value() instanceof RecipeDisplay 恒 false）。

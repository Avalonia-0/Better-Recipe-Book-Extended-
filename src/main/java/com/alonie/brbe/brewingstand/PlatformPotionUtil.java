...
public class PlatformPotionUtil {
    ...
    /** 配方消耗的<b>基底物品</b>（普通/喷溅/滞留药水中的哪一种）；本分支的数据
     *  不含物品形态（{@code PotionBrewing.Mix} 只有药水→药水）→ 恒 null。 */
    public static Item getInputItem(Object recipe) {
        return provider == null ? null : provider.getInputItem(recipe);
    }

    /** 配方产出的物品形态；本分支恒 null（形态由配方书标签页决定）。 */
    ...
    public interface PotionUtilProvider {
         ...
         * 1.21.11 的 {@code PotionBrewing.Mix} 只有"药水→药水"，**没有**物品
         * 形态（形态由配方书的标签页决定）。默认返回 null = 未知，调用方回退到
         * 标签页物品，保持本分支行为不变。
         */

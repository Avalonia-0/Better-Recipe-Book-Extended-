...
public final class PopupRenderer {
    ...
    private static void maskWithHoles(GuiGraphics gui, int x1, int y1, int x2, int y2,
                                      List<int[]> holes) {
     ...
     *  (highlighted when hovered), whole-area partial marking (with the owned
     *  material slots punched out) and the small slot icons — used by the
     *  query-viewer's buttons, whose popup is drawn by the separate popup
     *  layer. */

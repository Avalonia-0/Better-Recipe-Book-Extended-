...
public final class PopupRenderer {
    ...
    private static void maskSlot(GuiGraphics gui, int sx, int sy) {
     ...
     *  (highlighted when hovered), per-slot missing-material marking and the
     *  small slot icons — used by the query-viewer's buttons, whose popup is
     *  drawn by the separate popup layer. */
     ...
     *  otherwise its fitted native layout slots (INPUT slots consume the
     *  search space, OUTPUT slots are masked without consuming). */

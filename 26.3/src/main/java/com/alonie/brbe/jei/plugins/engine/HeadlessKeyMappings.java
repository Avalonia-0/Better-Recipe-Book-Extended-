...
public final class HeadlessKeyMappings implements IInternalKeyMappings {
    ...
    private static final IJeiKeyMappingInternal NONE = new IJeiKeyMappingInternal() {
        ...
        /** 26.3: {@link IJeiKeyMappingInternal} 新增 —— 无头场景没有真实 KeyMapping
         *  （不向游戏注册任何键位），给一个未绑定的占位对象。 */
        @Override
        public KeyMapping getKeyMapping() {
            return UNBOUND_KEY_MAPPING;
        }
    ...
    /** 26.3: 无头场景共享的占位 KeyMapping（未绑定、永不匹配）。 */
    private static final KeyMapping UNBOUND_KEY_MAPPING =
            new KeyMapping("key.jei.headless.none", InputConstants.UNKNOWN.getValue(), KeyMapping.Category.MISC);
    ...
    @Override
    public IJeiKeyMappingWithExtraModifiers getBookmark() { return NONE; }

    @Override
    public IJeiKeyMapping getToggleBookmarkOverlay() { return NONE; }

    @Override
    public IJeiKeyMappingWithExtraModifiers getShowRecipe() { return NONE; }

    @Override
    public IJeiKeyMappingWithExtraModifiers getShowUses() { return NONE; }
    ...
    /** 26.3: {@link IInternalKeyMappings} 新增 —— 需要向游戏注册的键位清单；
     *  无头场景不注册任何键位，返回空表。 */
    @Override
    public java.util.List<KeyMapping> getConfigKeyMappings() { return java.util.List.of(); }

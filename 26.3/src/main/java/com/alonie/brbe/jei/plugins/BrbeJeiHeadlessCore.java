...
public final class BrbeJeiHeadlessCore {
     ...
     *  entrypoint. */
    /** 26.3: 由客户端 RECIPE 注册表构建 {@link RecipeMap}（同步后的配方即在其中）。
     *  注册表/等级不可用时返回 {@link RecipeMap#EMPTY}，不抛异常。 */
    private static RecipeMap clientRecipeMap(Minecraft minecraft) {
        try {
            if (minecraft == null || minecraft.level == null) return RecipeMap.EMPTY;
            return minecraft.level.registryAccess()
                    .lookup(net.minecraft.core.registries.Registries.RECIPE)
                    .map(RecipeMap::create)
                    .orElse(RecipeMap.EMPTY);
        } catch (Throwable t) {
            LOGGER.warn("[BRBE-JEI-Plugins] failed to build client RecipeMap", t);
    ...
    public static void init() {
        ...
        ClientRecipeSynchronizedEvent.EVENT.register((minecraft, synchronizedRecipes) -> {
            // 26.3: RecipeMap 改为由**配方注册表 lookup** 构建
            // （RecipeMap.create(HolderLookup<Recipe<?>>)）——同步后的配方就写在
            // 客户端的 RECIPE 注册表里，因此从 registryAccess 取 lookup（与官方
            // VanillaClientRecipeLoader 同一路径）；注册表不可用时退化为 EMPTY。
            Internal.setClientSyncedRecipes(clientRecipeMap(minecraft));
    ...
    private static void start() {
        ...
        try {
            ...
            // 26.3: StartData 新增第三个组件 JeiConfigData（配置系统抽成独立
            // mod mezz_config 后由 JeiConfigRegistration.register() 统一构建，
            // 官方 JustEnoughItemsClient 也是这么接线的）。本工程内嵌了
            // mezz_config，因此这里走同一条注册路径。
            StartData startData = new StartData(
                    plugins, serverConnection, mezz.jei.library.config.JeiConfigRegistration.register());

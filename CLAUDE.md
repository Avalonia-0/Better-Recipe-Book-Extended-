...
- **预览/pin 红罩按槽位**（2026-08-21，两分支同步）：残缺/不可合成状态下，查询预览与 pin 界面不再整块红罩，改为按槽位标记——材料槽按渲染顺序扣减 `PartialCraftingUtil.searchSpaceCounts()`（拥有材料不画红罩，与合成台幽灵槽一致），缺失材料与结果槽画 `0x60FF3333`。`PopupGeometry.UnadaptedSlot` 携带 JEI role 序号（0=INPUT/1=OUTPUT）；crafting/synthetic/固定对（石材切割/锻造）/熔炉四种布局均覆盖。

...
- **预览/pin 残缺红罩整块挖洞**（2026-08-21，两分支同步）：残缺/不可合成状态下界面本体先盖整块红罩（`0x60FF3333`），`maskWithHoles` 矩形挖洞把已拥有材料的槽位格（16×16，不超出物品判定区域，按渲染顺序扣减 `PartialCraftingUtil.searchSpaceCounts()`）露出；`PopupGeometry.UnadaptedSlot` 携带 JEI role 序号（0=INPUT/1=OUTPUT），覆盖 crafting/synthetic/固定对/熔炉。

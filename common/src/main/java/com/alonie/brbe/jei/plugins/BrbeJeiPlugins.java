...
public final class BrbeJeiPlugins {
    ...
    /** Re-collect after each recipe-book rebuild (the engine's clear* path),
     *  so mod recipes are indexed once the synchronised recipe registry is full. */
     ...
     *  load).  Idempotent per join; re-runs on engine rebuilds. */
    public static void collectAndInject() {
        try {
            if (!rebuildListenerRegistered) {
                rebuildListenerRegistered = true;
                RecipeViewerEngine.addRebuildListener(BrbeJeiPlugins::collectAndInject);
            }
            ...
            for (IModPlugin plugin : plugins) {
                try {
                    ...
                    BetterRecipeBook.LOGGER.warn("[BRBE-JEI-Plugins] plugin {} failed: {}",

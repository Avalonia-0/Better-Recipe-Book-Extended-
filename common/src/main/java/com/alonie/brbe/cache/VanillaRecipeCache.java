...
public final class VanillaRecipeCache {
     ...
     * then re-injects valid cached entries — all or complement
     * depending on whether server entries exist.
     */
    ...
    static String extractResultItemId(SlotDisplay slot) {
        ...
        if (slot instanceof SlotDisplay.ItemStackSlotDisplay stackSlot) {
            return BuiltInRegistries.ITEM.getKey(stackSlot.stack().getItem()).toString();
        }
        return null;
    }

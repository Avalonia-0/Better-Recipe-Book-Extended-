...
public final class ConfigTipsHelper {
    ...
    public static Screen buildConfigScreen(Class configClass, Screen parent) {
        try {
            ...
            com.alonie.brbe.BetterRecipeBook.LOGGER.error("[BRBE] 配置界面条目整理失败，回退到未整理的默认顺序", e);
            try {
    ...
    /** RBIP 主开关：移到「§e查询合成/用途」文字行（{@code recipeViewerEnabled} 项）之前。 */
     ...
     * <ol>
     *   <li>「启用Recipe Book Is Pain」→「§e查询合成/用途」文字行之前；</li>
    ...
    private static void removeTitleBand(ClothConfigScreen cloth) {
        ...
        if (!titleBandLogged) {
            ...
            com.alonie.brbe.BetterRecipeBook.LOGGER.info("[BRBE] 配置界面顶部标题带已移除：标签 y {} → {}（shift={}），列表 top {} → {}",

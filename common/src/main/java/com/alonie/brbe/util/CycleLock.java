 ...
 *
 * <p>key 用物品自身的身份：按钮用 widget 实例、幽灵物品用
 *  {@code GhostRecipe$GhostIngredient} 实例、预览面板里的槽位用 {@link SlotKey}
 *  （配方身份 + 槽位序号，值相等）。</p>
 *
 * <h3>两层</h3>
...
public final class CycleLock {
    ...
    public static int cursorX() {
        return RecipeViewerOverlay.mouseXFor();
    }

    public static int cursorY() {
        return RecipeViewerOverlay.mouseYFor();
    }
     ...
     *
     *  <p>1.21.1 没有 RecipeDisplayId（那是 1.21.9+ 的显示体系），配方身份用
     *  {@code RecipeHolder}——它的 equals 是身份比较，所以同一配方两次取到的
     *  holder 必须来自同一来源（引擎条目 / 浮层按钮字段都是稳定实例）。</p> */
    public static Object slotKey(Object id, int slot) {
        return new SlotKey(id, slot);
    }

    /** {@link #slotKey} 的 key 类型。 */
    public record SlotKey(Object recipe, int slot) {
    }
}

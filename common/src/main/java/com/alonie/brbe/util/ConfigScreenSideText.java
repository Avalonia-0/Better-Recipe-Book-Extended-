 ...
 * {@code ClothConfigScreen$ListWidget} 覆写 {@code getItemWidth() = width - 80}，
 * 于是行左边界 = {@code 0 + width/2 - (width-80)/2 + 2 = 42}、行右边界 = {@code width - 38}，
 * 滚动条再占约 6px（{@code getScrollbarPosition() = width - 36}）。左右因此各留下一条
 * 约 42px / 30px 的空白竖条，正好放这种装饰文字（不会压到任何行内容）。</p>
 *
 ...
 *
 * <p><b>为什么每帧重算</b>：布局只由字体度量与当前屏幕尺寸决定，几十次 {@code font.width(...)}
 * 的开销可以忽略；换来的是**窗口缩放 / GUI Scale 变化自动跟随**，不必监听 resize
 * （Cloth 的列表尺寸在每次 {@code init()} 里重新算好，我们直接读当前值）。</p>
 *
 ...
 * {@code ScreenEvent.Render.Post}。{@link #shouldRender(Screen)} 用于入口侧的过滤。</p>
 */
public final class ConfigScreenSideText {
    ...
    /** 文字颜色（ARGB）。半透明可以当水印；不想要阴影，像素字更干净。 */
    private static final int TEXT_COLOR = 0xB0A0A0A0;
    /** 是否带阴影（原版字体阴影）。 */
    ...
    public static void render(Screen screen, GuiGraphics gui, int mouseX, int mouseY, float partialTick) {
        ...
        drawColumn(gui, mc.font, LEFT_TEXT, list.left, rowLeft, top, bottom, LEFT_SPACING);
        drawColumn(gui, mc.font, RIGHT_TEXT, rowRight + 2, list.right, top, bottom, RIGHT_SPACING);
    }

    /**
     * 在一条竖条内自上而下逐字符画 {@code text}：
    ...
    private static void drawColumn(GuiGraphics gui, Font font, String text,
                                   ...
                                   int top, int bottom, float spacing) {
        int[] cps = text.codePoints().toArray();
        if (cps.length == 0 || stripRight <= stripLeft) return;

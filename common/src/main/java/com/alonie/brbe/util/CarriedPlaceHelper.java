...
public final class CarriedPlaceHelper {
    ...
    private static boolean clearGridToCarried(AbstractContainerMenu menu, List<Slot> gridSlots,
        ...
        for (Slot slot : gridSlots) {
            ...
            if (carried.isEmpty()) {
                // 空手拿起整堆
                gameMode.handleContainerInput(containerId, slot.index, 0, ContainerInput.PICKUP, player);
                continue;
            }
            ...
            gameMode.handleContainerInput(containerId, slot.index, 0, ContainerInput.QUICK_MOVE, player);
    ...
    private static boolean placeGrid(AbstractContainerMenu menu, List<Placement> placements,
        ...
        for (Placement p : placements) {
            ...
            if (p.candidates.contains(carried.getItem())) {
                gameMode.handleContainerInput(containerId, p.slot.index, 1, ContainerInput.PICKUP, player);
            }
        }
        ...
        if (!menu.getCarried().isEmpty()) {
            ...
            gameMode.handleContainerInput(containerId, stash.index, 0, ContainerInput.PICKUP, player);
    ...
    private static boolean fillRemainingFromInventory(AbstractContainerMenu menu, List<Placement> placements,
        ...
        for (Placement p : placements) {
            ...
            gameMode.handleContainerInput(containerId, inv.index, 0, ContainerInput.PICKUP, player);
            gameMode.handleContainerInput(containerId, p.slot.index, 1, ContainerInput.PICKUP, player);
            if (!menu.getCarried().isEmpty()) {
                gameMode.handleContainerInput(containerId, inv.index, 0, ContainerInput.PICKUP, player);
            }
